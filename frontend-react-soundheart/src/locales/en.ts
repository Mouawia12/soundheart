/**
 * UI chrome strings (nav, buttons, form labels, dashboard). Long-form marketing
 * copy lives in the CMS. `ar.ts` mirrors this exact shape for the Arabic build.
 */
const en = {
  lang: {
    // Shown on the toggle: the language you switch TO.
    switchTo: 'العربية',
    label: 'Language',
  },
  nav: {
    about: 'About',
    services: 'Services',
    model: 'The Model',
    therapy: 'Therapy',
    retreats: 'Retreats',
    training: 'Training',
    resources: 'Resources',
    shop: 'Shop',
    contact: 'Contact',
    clientLogin: 'Client Login',
    getStarted: 'Get Started',
    book: 'Book a consultation',
  },
  guideNav: {
    services: 'Services',
    articles: 'Articles',
    book: 'Book a session',
  },
  common: {
    bookConsultation: 'Book a consultation',
    bookSession: 'Book a session',
    read: 'Read',
    readArticle: 'Read the article',
    viewAllArticles: 'View all articles',
    browseAllArticles: 'Browse all articles →',
    menu: 'Menu',
    home: 'Home',
    backHome: '← Back home',
  },
  floating: {
    whatsapp: 'Message us on WhatsApp for general inquiries',
    whatsappTip: 'WhatsApp us · general inquiries only',
    book: 'Book a consultation',
  },
  footer: {
    basedIn: 'Based in the Mat-Su Valley',
    workingWith: 'Working with clients across the lower 48 and beyond',
    contact: 'Contact',
    faq: 'FAQ',
    privacy: 'Privacy',
    terms: 'Terms',
    disclaimer: 'Disclaimer',
    clientLogin: 'Client Login',
  },
  contact: {
    sendMessage: 'Send us a message',
    firstName: 'First name',
    lastName: 'Last name',
    email: 'Email',
    phoneOptional: 'Phone (optional)',
    whatBrings: 'What brings you here?',
    aboutMessage: "A little about what's going on",
    send: 'Send message',
    sending: 'Sending…',
    readyToBegin: 'Ready to begin?',
    preferToTalk: 'Prefer to talk? Call',
    orEmail: 'or email',
  },
  booking: {
    eyebrow: 'Book a session',
    title: 'Choose a time that works for you',
    lead: "Pick a day and time below. You'll get a confirmation by email; for online sessions it includes a video link.",
    loadingAvailability: 'Loading availability…',
    availableTimes: 'Available times',
    loadingTimes: 'Loading times…',
    noTimes: 'No times available on this day. Try another date.',
    selectDay: 'Select a day to see available times.',
    booked: "You're booked ✓",
    confirmationTo: 'A confirmation is on its way to',
    yourEmailWord: 'your email',
    andGuests: 'and your guests',
    onlineIncludes: 'For online sessions it includes your video link.',
    paymentCancelled: "Payment was cancelled — your session wasn't booked. Pick a time to try again.",
    changeTime: '← Change time',
    yourSession: 'Your session',
    online: 'Online',
    inPerson: 'In person',
    yourName: 'Your name',
    yourEmail: 'Your email',
    emailHint: "We'll send the confirmation and (for online) the video link here.",
    guests: 'Guests (optional)',
    addGuest: '+ Add a guest email',
    submitting: 'Booking…',
    confirm: 'Confirm booking',
    errName: 'Please enter your name.',
    errEmail: 'Please enter your email.',
    errGeneric: 'Could not complete the booking. Please try another time.',
  },
  auth: {
    adminSignIn: 'Admin sign in',
    subtitle: 'Manage articles, categories, and content.',
    email: 'Email',
    password: 'Password',
    signIn: 'Sign in',
    signingIn: 'Signing in…',
    showPassword: 'Show password',
    hidePassword: 'Hide password',
    signInFailed: 'Sign in failed',
  },
  admin: {
    nav: {
      dashboard: 'Dashboard',
      articles: 'Articles',
      categories: 'Categories',
      pages: 'Pages',
      bookings: 'Bookings',
      bookingSettings: 'Booking settings',
      settings: 'Settings',
    },
    signOut: 'Sign out',
    viewSite: '↗ View site',
    save: 'Save changes',
    saving: 'Saving…',
    saved: 'Saved ✓',
    back: '← Back',
    cancel: 'Cancel',
    dashboard: {
      title: 'Dashboard',
      newArticle: '+ New article',
      articles: 'Articles',
      published: 'Published',
      drafts: 'Drafts',
      categories: 'Categories',
      recent: 'Recent articles',
    },
    pages: {
      title: 'Pages',
      intro: "Edit each page's hero text, photos, content, and call-to-action. Changes appear on the live site.",
      groupMain: 'Main pages',
      groupServices: 'Local service pages',
      groupGuides: 'Guides',
      edit: 'Edit →',
    },
    settings: {
      title: 'Site settings',
      account: 'Account',
      changePassword: 'Change password',
    },
  },
} as const

export default en
