 ...
 * Brand colours (navy/gold), not the default WhatsApp green. Pinned to the
 * inline-end side (right in LTR, left in RTL).
 */
export default function FloatingContact() {
  const { t } = useTranslation()
  return (
    <div className="fixed bottom-[18px] right-[18px] z-[9999] flex flex-col gap-3 font-sans rtl:left-[18px] rtl:right-auto">
      <a
      ...
      >
        <span className="pointer-events-none absolute right-[66px] top-1/2 -translate-y-1/2 whitespace-nowrap rounded-lg bg-navy px-[11px] py-1.5 text-[0.78rem] font-bold text-ivory opacity-0 shadow-[0_6px_16px_-6px_rgba(0,0,0,0.4)] transition-opacity group-hover:opacity-100 rtl:left-[66px] rtl:right-auto">
          {t('floating.whatsappTip')}
        </span>
